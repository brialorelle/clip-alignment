...
all_subject_number_list = english_subject_list
...
def process_video(mp4_full_path, subject):
    ...
    command = f"python3 {cwd_path}/batch_clip_on_all_frames_adult_only.py --video_file {mp4_full_path} --csv_file {transcript_full_path} --save_root_dir {output_full_path} --frame_batch_size {frame_batch_size} --clip_batch_size {clip_batch_size} --prefetch {prefetch}"
    os.system(command)
